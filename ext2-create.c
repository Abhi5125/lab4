...
void write_superblock(int fd) {
	...
	superblock.s_inodes_per_group = superblock.s_blocks_per_group ? NUM_INODES / (NUM_BLOCKS / superblock.s_blocks_per_group) : 0;
	superblock.s_mtime = 0;				/* Mount time */
	superblock.s_wtime = current_time;	/* Write time */
	superblock.s_mnt_count         = 0; /* Number of times mounted so far */
	superblock.s_max_mnt_count     = INT64_MAX; /* Make this unlimited */
